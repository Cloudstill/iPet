...
- **Local SQLite persistence with versioned migrations** — preferences, chat history, tool configs, token usage, system samples, disk-scan cache; schema evolves via `PRAGMA user_version` (currently v2).
...
- **81 tests** (51 host + 8 scan-disk crate + 22 frontend) covering config validation, disk scanner, storage + migrations, tool-package parsing, dispatcher (incl. local subprocess), HTTP safety, secret encryption, Markdown sanitization, the three-mode shell + Control Center section render dispatch, and chat message-type rendering (assistant / user / tool-event / system-event / error).
...
- **本地 SQLite 持久化 + 版本化迁移** —— 偏好、聊天历史、工具配置、token 使用、系统采样、磁盘扫描缓存；schema 通过 `PRAGMA user_version` 演进（当前 v2）。
- **API Key 本机加密** —— LLM API key 用 ChaCha20-Poly1305 + 每机密钥加密落库，非明文。
- **Token 统计** —— 按天 / 按模型 / 最近请求拆分；同一次对话里工具决策调用和最终流式回复的 usage 会合并成一条记录。
- **Cargo workspace** —— 两个内置工具作为 `tool-packages/*/rust/` crate 被 `src-tauri` 依赖，运行时与可分发包共用同一份真源。
- **60 个测试**（51 主机 + 8 scan-disk crate + 11 前端），覆盖配置校验、磁盘扫描、存储与迁移、工具包解析、dispatcher（含 local 子进程）、HTTP 安全、密钥加密、Markdown 清洗。
